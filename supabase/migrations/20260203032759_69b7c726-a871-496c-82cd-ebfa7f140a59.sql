-- Fix function search_path for get_user_rankings
...
BEGIN
  ...
  WITH user_stats AS (
    ...
    -- Pontos de conquistas resgatadas no mês atual
    LEFT JOIN (
    ...
    -- CORREÇÃO: Logins TOTAIS (histórico completo, não apenas do mês)
    LEFT JOIN (
  ...
  SELECT 
    ...
    (us.schedule_points + us.progress_points + us.quiz_points + us.devotional_points + us.achievement_points)::bigint AS total_points,
    us.active_days,
    ROW_NUMBER() OVER (ORDER BY (us.schedule_points + us.progress_points + us.quiz_points + us.devotional_points + us.achievement_points) DESC, us.active_days DESC)::bigint AS rank
  FROM user_stats us
  WHERE (us.schedule_points + us.progress_points + us.quiz_points + us.devotional_points + us.achievement_points) > 0
  ORDER BY rank;
END;
$function$;