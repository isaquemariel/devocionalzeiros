...
-- Recreate admin_get_metrics with embaixador_plans column
...
BEGIN
  ...
  -- Count embaixador users separately
  SELECT COUNT(DISTINCT ap.user_id) INTO embaixador_count 
  FROM authorized_purchases ap
  WHERE ap.status = 'active' 
    AND ap.user_id IS NOT NULL
    AND ap.plan_type = 'embaixador';
  ...
  RETURN QUERY SELECT
    ...
    -- Gratuito users = registered users minus paying users minus embaixador users
    (registered_users_count - paying_users_count - embaixador_count)::bigint as gratuito_users,
