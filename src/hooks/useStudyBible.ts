...
// Fallback verses for offline/error scenarios
function generateFallbackVerses(bookId: string, chapter: number): Verse[] {
  ...
  if (fallbackData[bookId]?.[chapter]) {
  ...
  return Array.from({ length: verseCount }, (_, i) => ({
    number: i + 1,
    text: `Versículo ${i + 1} de ${book?.name || bookId} capítulo ${chapter}. Conecte-se à internet para carregar o texto completo.`,
  }));
}