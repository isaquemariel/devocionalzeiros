...
import { Sparkles, BookOpen, Heart, MessageCircle, CheckCircle2, Flame, Calendar, Trophy, Loader2, ArrowLeft, Star, Quote, Feather, Share2 } from "lucide-react";
...
const Devocional = () => {
  ...
  const {
    ...
    isFavorite,
    toggleFavorite
  } = useDevotionalFavorites(user?.id);
  ...
  const formattedDate = useMemo(() => {
    ...
    return format(selectedDate, "d 'de' MMMM, yyyy", {
      locale: ptBR
    });
  }, [selectedDate]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const {
          data: completions
        } = await supabase.from("devotional_completions").select("devotional_date").eq("user_id", user.id).order("devotional_date", {
          ascending: false
        });
        if (completions) {
          ...
          const sortedDates = completions.map(c => new Date(c.devotional_date)).sort((a, b) => a.getTime() - b.getTime());
  ...
  return <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      <div className="relative z-10 max-w-4xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6">
        {/* Header */}
        <AppHeader userId={user?.id} userEmail={user?.email} showBack={true} showLogo={true} />

        <AnimatePresence mode="wait">
          {showCalendar ? <motion.div key="calendar" initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
          ...
          duration: 0.3
              ...
              {/* Stats Cards */}
              <motion.div className="grid grid-cols-3 gap-3 mb-6" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.1
              ...
              <DevotionalCalendar onSelectDate={handleSelectDate} availableDays={AVAILABLE_DEVOTIONAL_DAYS} completedDates={completedDates} favorites={favorites} />
            </motion.div> : selectedDevotional ? <motion.div key="devotional" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          duration: 0.4
              ...
              {/* Back Button */}
              <motion.button onClick={handleBackToCalendar} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors mb-6" whileHover={{
            x: -4
          }}>
              ...
              <div className="animate-page-turn">
                <div className="notebook-page relative">
                  ...
                  <div className="text-center pb-6 border-b border-amber-200/50 dark:border-amber-800/30">
                    <motion.div className="inline-flex items-center gap-2 text-amber-600/70 dark:text-amber-400/60 mb-3" initial={{
                    opacity: 0,
                    y: -10
                  }} animate={{
                    opacity: 1,
                    ...
                    <motion.h1 className="text-2xl sm:text-3xl font-serif font-bold text-stone-800 dark:text-amber-100 mb-4" initial={{
                    opacity: 0,
                    y: 10
                  }} animate={{
                    opacity: 1,
                    ...
                    {isCompleted && <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" initial={{
                    scale: 0
                  }} animate={{
                    scale: 1
                  }} transition={{
                  ...
                  {/* Verse Section */}
                  <motion.section className="space-y-3" initial={{
                  opacity: 0,
                  x: -20
                }} animate={{
                  opacity: 1,
                  ...
                  {/* Prayer Section */}
                  <motion.section className="space-y-3 p-4 sm:p-5 rounded-xl bg-blue-50/50 dark:bg-blue-950/20 border-l-4 border-blue-400 dark:border-blue-600" initial={{
                  opacity: 0,
                  x: -20
                }} animate={{
                  opacity: 1,
                  ...
                  {/* Phrase of the Day */}
                  <motion.section className="space-y-4 text-center py-6" initial={{
                  opacity: 0,
                  scale: 0.95
                }} animate={{
                  opacity: 1,
                  ...
                  <motion.div className="pt-6 border-t border-amber-200/50 dark:border-amber-800/30" initial={{
                  opacity: 0,
                  y: 20
                }} animate={{
                  opacity: 1,
                  ...
                  delay: 0.6
                    ...
                    <div className="flex gap-3">
                      {/* Complete Button */}
                      <motion.button onClick={handleComplete} disabled={isCompleted} className={`flex-1 py-4 rounded-xl font-medium text-base transition-all ${isCompleted ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 cursor-default" : "bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 hover:scale-[1.02]"}`} whileHover={!isCompleted ? {
                      scale: 1.02
                    } : {}} whileTap={!isCompleted ? {
                      scale: 0.98
                    } : {}}>
                        {isCompleted ? <span className="flex items-center justify-center gap-2">Concluído ✔<CheckCircle2 className="w-5 h-5" />
                      ...
                      {/* Favorite Button */}
                      <motion.button onClick={handleToggleFavorite} disabled={isFavoriting} className={`p-4 rounded-xl transition-all ${selectedDayOfYear && isFavorite(selectedDayOfYear) ? "bg-rose-500 text-white shadow-[0_0_20px_rgba(244,63,94,0.6)]" : "bg-rose-500/20 text-rose-400 border border-rose-500/30 hover:bg-rose-500/30"}`} whileHover={{
                      scale: 1.05
                    }} whileTap={{
                      scale: 0.95
                    }} title={selectedDayOfYear && isFavorite(selectedDayOfYear) ? "Remover dos favoritos" : "Adicionar aos favoritos"}>
                        <Heart className={`w-5 h-5 ${selectedDayOfYear && isFavorite(selectedDayOfYear) ? "fill-white" : ""}`} />
                      </motion.button>

                      {/* Share Button */}
                      <motion.button onClick={handleOpenShareModal} className="p-4 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-[0_0_20px_rgba(59,130,246,0.6),0_0_40px_rgba(59,130,246,0.3)] hover:shadow-[0_0_30px_rgba(59,130,246,0.8),0_0_60px_rgba(59,130,246,0.4)] transition-all" whileHover={{
                      scale: 1.05
                    }} whileTap={{
                      scale: 0.95
                    }} animate={{
                      scale: [1, 1.1, 1],
                      boxShadow: ["0 0 20px rgba(59,130,246,0.6), 0 0 40px rgba(59,130,246,0.3)", "0 0 30px rgba(59,130,246,0.8), 0 0 60px rgba(59,130,246,0.5)", "0 0 20px rgba(59,130,246,0.6), 0 0 40px rgba(59,130,246,0.3)"]
                    }} transition={{
                      duration: 1.2,
                      repeat: Infinity,
                      ease: "easeInOut"
                    ...
                    {!isCompleted && <motion.p className="text-center text-sm text-stone-500 dark:text-stone-500 mt-4" initial={{
                    opacity: 0
                  }} animate={{
                    opacity: 1
                  }} transition={{
        ...
        {/* Share Modal */}
        <ShareOptionsModal isOpen={showShareModal} onClose={handleCloseShareModal} imagePreview={imagePreview} isGenerating={isGenerating} onShareWhatsApp={shareToWhatsApp} onDownload={downloadImage} />

        {/* Hidden Shareable Card for Image Generation */}
        {selectedDevotional && selectedDate && <div style={{
        position: "absolute",
        left: "-9999px",
        top: "-9999px"
      }}>
            <ShareableDevotionalCard ref={cardRef} title={selectedDevotional.title} verse={selectedDevotional.verse} meditation={selectedDevotional.meditation} date={selectedDate} />
          </div>}
      </div>
    </div>;
};
