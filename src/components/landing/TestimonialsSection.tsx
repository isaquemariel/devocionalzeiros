  ...
  return <motion.div initial={{
    ...
    once: true
      ...
      <div className="relative aspect-[9/16] w-full max-w-[280px] mx-auto rounded-2xl overflow-hidden bg-secondary/30 border border-white/10 shadow-2xl">
        ...
        {!isPlaying && <div className="absolute inset-0 z-20 cursor-pointer group/play" onClick={handlePlay}>
            {/* Thumbnail from YouTube */}
            <img src={`https://img.youtube.com/vi/${testimonial.videoId}/maxresdefault.jpg`} alt={`Depoimento de ${testimonial.name}`} className="w-full h-full object-cover" onError={e => {
        ...
        {isPlaying && <>
            <iframe ref={iframeRef} src={embedUrl} className="absolute inset-0 w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={`Depoimento de ${testimonial.name}`} />
