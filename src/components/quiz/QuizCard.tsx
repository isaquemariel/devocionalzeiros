...
export const QuizCard = ({
  ...
  return (
    <motion.div
      className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-600/20 via-violet-600/15 to-indigo-600/20 border border-purple-500/30 p-5 cursor-pointer hover:border-purple-500/50 transition-all group"
    ...
    >
      {/* Glow Effect */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-purple-500/20 rounded-full blur-[50px] group-hover:bg-purple-500/30 transition-colors" />
      ...
      <div className="relative z-10">
        ...
        {chaptersReadToday > 0 ? (
          <div className="space-y-2 mb-4">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold text-purple-400">{chaptersReadToday}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Perguntas disponíveis</span>
              <span className="font-semibold text-purple-400">{questionsRemaining}</span>
            </div>
          </div>
        ) : (
        ...
        {/* Action Button */}
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-2 px-4 rounded-lg bg-purple-500/20 text-purple-300">
            <Loader2 className="w-4 h-4 animate-spin" />
            <span className="text-sm">Carregando...</span>
          </div>
        ) : hasQuestionsAvailable && chaptersReadToday > 0 ? (
          <div className="flex items-center justify-between py-2 px-4 rounded-lg bg-purple-500/20 text-purple-300 group-hover:bg-purple-500/30 transition-colors">
